...
if(UNIX)
    if(DEFINED ENV{CC})
        ...
        elseif(CC STREQUAL "emcc")
            ...
            string(REGEX REPLACE ".*gcc\\/clang\\-like replacement\\) ([0-9]+\\.[0-9]+).*" "\\1" CC_VERSION ${CC_VERSION_VERBOSE})
            set(CC_ARCH "asmjs")  
            set(CC_PLATFORM "web")
        else()
            ...
            if(CC_CLANG GREATER_EQUAL 0)
                if(APPLE)
                    ...
                    string(REGEX REPLACE ".*Xcode ([0-9]+\\.[0-9]+).*" "\\1" CC_VERSION ${CC_VERSION_VERBOSE})
                else()
                    set(CC_NAME "clang")
                    exec_program($ENV{CC} ARGS --version OUTPUT_VARIABLE CC_VERSION_VERBOSE)
                    string(REGEX REPLACE ".*clang version ([0-9]+\\.[0-9]+).*" "\\1" CC_VERSION ${CC_VERSION_VERBOSE})
                endif()
                exec_program($ENV{CC} ARGS -dumpmachine OUTPUT_VARIABLE CC_ARCH)
                set(CC_PLATFORM "${CC_ARCH}")
            endif()
    ...
    set(CC_NAME "msvc")
    set(CC_PLATFORM "windows")
    exec_program(cl OUTPUT_VARIABLE CC_VERSION_VERBOSE)
    string(FIND "${CC_VERSION_VERBOSE}" "19.10" CC_VERSION_2017)
...
if(CC_NAME STREQUAL "msvc" OR CC_NAME STREQUAL "clang")
    ...
    elseif(CC_NAME STREQUAL "clang")
        if(DEFINED ENV{CMAKE_TOOLCHAIN_FILE})
            ctest_configure(OPTIONS "-DBUILD_TESTING=Off -DCI_FLAGS=--analyze -DCMAKE_TOOLCHAIN_FILE=$ENV{CMAKE_TOOLCHAIN_FILE}")
        else()
            ctest_configure(OPTIONS "-DBUILD_TESTING=Off -DCI_FLAGS=--analyze")
        endif()
    endif()
...
if(CTEST_MEMORYCHECK_TYPE STREQUAL "AddressSanitizer")
    ...
    ctest_read_custom_files(${CTEST_SOURCE_DIRECTORY})
    if(DEFINED ENV{CMAKE_TOOLCHAIN_FILE})
        ctest_configure(OPTIONS "-DCI_FLAGS=${CC_ASAN_FLAGS} -fsanitize=address -DCMAKE_TOOLCHAIN_FILE=$ENV{CMAKE_TOOLCHAIN_FILE}")
    else()
...
if(CTEST_MEMORYCHECK_TYPE STREQUAL "MemorySanitizer")
    ...
    ctest_read_custom_files(${CTEST_SOURCE_DIRECTORY})
    if(DEFINED ENV{CMAKE_TOOLCHAIN_FILE})
        ctest_configure(OPTIONS "-DCI_FLAGS=-fsanitize=memory -fsanitize-memory-track-origins -DCMAKE_TOOLCHAIN_FILE=$ENV{CMAKE_TOOLCHAIN_FILE}")
    else()
...
if(CTEST_MEMORYCHECK_TYPE STREQUAL "ThreadSanitizer")
    ...
    ctest_read_custom_files(${CTEST_SOURCE_DIRECTORY})
    if(DEFINED ENV{CMAKE_TOOLCHAIN_FILE})
        ctest_configure(OPTIONS "-DCI_FLAGS=${CC_TSAN_FLAGS} -fsanitize=thread -DCMAKE_TOOLCHAIN_FILE=$ENV{CMAKE_TOOLCHAIN_FILE}")
    else()
